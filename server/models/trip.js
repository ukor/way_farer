class Trip {
  ...
  async save(tripDetails) {
    ...
    const values = [
      this.td.slug, this.td.bus_slug,
      this.td.origin, this.td.destination,
      this.td.trip_date, this.td.fare,
      this.td.created_by, this.td.status,
  ...
  fetch(filterType = 'origin' || 'destination' || 'id' || 'bus', filter) {
    this.filter = filter;
    this.ft = filterType;
  }
